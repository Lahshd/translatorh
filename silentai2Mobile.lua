...
StatusLabel.Text = "Status: ACTIVE\nListening exclusively for $commands"
...
-- === MESSAGE PROCESSOR (EXCLUSIVELY PREFIX BASED) ===
local function processIncomingMessage(player, messageText)
    if not botEnabled then return end
    if player == LocalPlayer then return end

    -- Strictly check if message starts with '$'
    if not messageText:sub(1, 1) == "$" and not messageText:find("^%$") then
        return
    end

    local cleanCommand = messageText:gsub("^%$", ""):gsub("^%s*", "")
    local lowerCmd = cleanCommand:lower()
    local senderName = player.DisplayName or player.Name

    -- 1. Utility & Mode Commands
    if lowerCmd == "stop" then
        stopNavigation()
        sendMessage("Stopped movement! ♡")
        return
    elseif lowerCmd == "owo" then
        currentModeIndex = 1
        ModeBtn.Text = "Mode: OwO Mode"
        sendMessage("Switched to OwO mode! >w<")
        return
    elseif lowerCmd == "tsundere" then
        currentModeIndex = 2
        ModeBtn.Text = "Mode: Tsundere Mode"
        sendMessage("B-Baka! Switched to Tsundere mode!")
        return
    elseif lowerCmd == "yandere" then
        currentModeIndex = 3
        ModeBtn.Text = "Mode: Yandere Mode"
        sendMessage("Switched to Yandere mode... ♡")
        return
    elseif lowerCmd == "mode" then
        currentModeIndex = (currentModeIndex % #Modes) + 1
        ModeBtn.Text = "Mode: " .. Modes[currentModeIndex].Name
    ...
    -- 2. Movement Commands
    if lowerCmd == "follow" or lowerCmd == "follow me" or lowerCmd == "come here" then
        startFollowing(player)
    elseif lowerCmd:find("^follow%s+") or lowerCmd:find("^goto%s+") then
        local targetName = lowerCmd:match("follow%s+(%w+)") or lowerCmd:match("goto%s+(%w+)")
        if targetName then
    ...
    -- 3. Raycast & Pathfinding Object Interacting Commands ($goto, $go onto, $sit)
    ...
    -- 4. AI Chat Query Execution
    if isProcessing then
        sendMessage(Modes[currentModeIndex].ThinkingMsg)
        return
    end

    -- Add Spatial Raycast Context to Prompt
    local processedPrompt = cleanCommand
    if detectedObj then
